...
<div class="test-box">
					...
					<p>5.2凡因本协议引起的或与本协议有关的任何争议，由双方友好协商解决。协商不成时，任何一方均应向合同签订地法院（即：南京市玄武区人民法院）提起诉讼。</p>
                        ...
                        <div class="panel-footer" style="text-align:center;">
                            <button type="button" class="btn  btn-warning btn-lg" id="startPro">阅读并同意协议</button>
                        </div>
                    </div>
                </div>
            </div>
 ...
 <%@include file="/WEB-INF/include/commons-js.jsp" %>
	<script>
		...
		$("#startPro").click(function () {
		    ...
		    $.ajax({
				url:"http://localhost:8081/api/project/startPro?memberId=${login.id}",
				type:"post",
				success:function (data) {
					console.log(data);
					sessionStorage.pid = data.ext.pid;
					console.log(sessionStorage.pid);
					window.location.href = "/start-step-1.html";
                }
			});
